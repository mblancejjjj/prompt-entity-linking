...
class RerankNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # query_embed = query_embed['pooler_output'].unsqueeze(1)
        # breakpoint()
        # first_layer = query_embed['hidden_states'][1][:,0]
        # last_layer = query_embed['hidden_states'][-1][:,0]
        # first_layer = query_embed['hidden_states'][1]
        # last_layer = query_embed['hidden_states'][-1]
        # seq_length = first_layer.shape[1]

        # first_avg = F.avg_pool1d(first_layer.transpose(1,2), kernel_size=seq_length).squeeze(-1)
        # last_avg = F.avg_pool1d(last_layer.transpose(1,2), kernel_size=seq_length).squeeze(-1)
        # merge_embeds = torch.cat([first_avg, last_avg], dim=1)
        # # merge_embeds = torch.cat([first_layer, last_layer], dim=1)
        # query_embed = F.avg_pool1d(merge_embeds, kernel_size=2).unsqueeze(1) 
        ...
        # breakpoint()
        # first last merge
        # first_layer = candidate_embeds['hidden_states'][1][:,0]
        # last_layer = candidate_embeds['hidden_states'][-1][:,0]

        # first_layer = candidate_embeds['hidden_states'][1]
        # last_layer = candidate_embeds['hidden_states'][-1]

        # first_avg = F.avg_pool1d(first_layer.transpose(1,2), kernel_size=seq_length).squeeze(-1)
        # last_avg = F.avg_pool1d(last_layer.transpose(1,2), kernel_size=seq_length).squeeze(-1)
        # # merge_embeds = torch.cat([first_layer, last_layer], dim=1)
        # merge_embeds = torch.cat([first_avg, last_avg], dim=1)
        # candidate_embeds = F.avg_pool1d(merge_embeds, kernel_size=2).reshape(batch_size, topk, -1)
        # breakpoint()
        candidate_embeds = candidate_embeds[0][:,0].reshape(batch_size, topk, -1) # [batch_size, topk, hidden]
        # candidate_embeds = candidate_embeds['pooler_output'].reshape(batch_size, topk, -1)
        # score dense candidates
        # candidate_d_score = torch.bmm(query_embed, candidate_embeds.permute(0,2,1)).squeeze(1)
        candidate_d_score = self.linear1(candidate_embeds).squeeze(-1)
        # breakpoint()
