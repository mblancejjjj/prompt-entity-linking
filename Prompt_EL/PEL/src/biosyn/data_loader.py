...
class QueryDataset(Dataset):
    ...
    def load_ncbi_data(self, data_dir, filter_composite, filter_duplicate, filter_cuiless):
        data = []
        with open(data_dir, "r", encoding='utf-8') as f:
            concepts = f.readlines()
        for concept in concepts:
            concept = concept.split("\t")
            mention = concept[1].strip()
            cui = concept[2].strip()
    ...
    def load_data(self, data_dir, filter_composite, filter_duplicate, filter_cuiless):
        ...
        for concept_file in tqdm(concept_files):
            with open(concept_file, "r", encoding='utf-8') as f:
                concepts = f.readlines()
            # '10747931||138|145||DiseaseClass||lipomas||D008067\n'
            print(concept_file)
            for concept in concepts:
                ...
                #data.append((mention, cui, context))
                data.append((mention, cui))
        if filter_duplicate:
            data = list(dict.fromkeys(data))
...
class DictionaryDataset():
    ...
    def __init__(self, dictionary_path):
        ...
        # self.data = self.load_ncbi_data(dictionary_path)

    def load_ncbi_data(self, dictionary_path):
        name_cui_map = {}
        data = []
...
class CandidateDataset(Dataset):
    ...
    def __getitem__(self, query_idx):
        ...
        candidate_tokens = self.tokenizer([query_name]*len(candidate_names),text_pair=candidate_names, max_length=self.max_length, padding='max_length', truncation=True, return_tensors='pt')
        # breakpoint()
